...
[mooring_data.GE_dox2_molm2, mooring_data.dox2_gas_exchange_molm2, mooring_data.dox2_bubbles_molm2,mooring_data.dox2_eddy_diffusion_molm2,mooring_data.dox2_entrainment_molm2] = deal(zeros(size(mooring_data.time)));
...
for i = 2:length(mooring_data.time)
    ...
    % The effect of eddy diffusion at the current timestamp is calculated,
    % using a coefficient, the difference between mld and submld oxygen, and 
    % the user defined thickness of the transition layer between the mixed 
    % layer and deep water, which determines the gradient, following the 
    % form of the equation in King and Devol (1979).
    
    mooring_data.dox2_eddy_diffusion_molm2(i) = 3600*eddy_diff_coeff*((mooring_data.sub_mld_dox2_molm3(i-1))-mooring_data.dox2_molm3(i-1))/eddy_gradient_thickness_m;
    
    
    % Entrainment is calculated by multiplying the difference in oxygen between
    % the submld and mld oxygen values, by the deeping of the mld over the 
    % previous hour.
    
    mooring_data.dox2_entrainment_molm2(i) = ((mooring_data.sub_mld_dox2_molm3(i-1))-mooring_data.dox2_molm3(i-1))*(mooring_data.mld_diff(i-1));
    ...
    mooring_data.dox2_biology_molm2(i) = mooring_data.mld_smooth(i)*(mooring_data.dox2_molm3(i)-mooring_data.dox2_molm3(i-1)) - mooring_data.dox2_gas_exchange_molm2(i) - mooring_data.dox2_bubbles_molm2(i) - mooring_data.dox2_eddy_diffusion_molm2(i) - mooring_data.dox2_entrainment_molm2(i);
...
[mooring_data.GE_dox2_phys_molm3, mooring_data.dox2_phys_gas_exchange_molm3, mooring_data.dox2_phys_bubbles_molm3,mooring_data.dox2_phys_eddy_diffusion_molm3, mooring_data.dox2_phys_molm3] = deal(zeros(size(mooring_data.time)));
...
for i = 2:length(mooring_data.time)
    ...
    mooring_data.dox2_phys_eddy_diffusion_molm3(i) = (3600/mooring_data.mld_smooth(i))*eddy_diff_coeff*((mooring_data.sub_mld_dox2_molm3(i-1))-mooring_data.dox2_phys_molm3(i-1))/eddy_gradient_thickness_m;
    ...
    mooring_data.dox2_phys_entrainment_molm3(i) = ((mooring_data.sub_mld_dox2_molm3(i))-mooring_data.dox2_phys_molm3(i-1))*(mooring_data.mld_diff(i-1))/mooring_data.mld_smooth(i);
    ...
    mooring_data.dox2_phys_molm3(i) = mooring_data.dox2_phys_molm3(i-1) + mooring_data.dox2_phys_bubbles_molm3(i) + mooring_data.dox2_phys_gas_exchange_molm3(i) + mooring_data.dox2_phys_eddy_diffusion_molm3(i) + mooring_data.dox2_phys_entrainment_molm3(i);
